...
export default function Product({ product }) {
  if (!product) return null; // <-- error fix

  return (
    <div className="bg-white shadow rounded-lg overflow-hidden hover:shadow-lg flex flex-col transition-all w-full  ">
      <Link href={`/shop/${product.slug}`}>  
        {/* Image */}
        <div className="w-full bg-gray-50">
          <img
            src={product.images?.[0] || "/placeholder.png"} // <-- error fix
            ...
            <div className="flex flex-wrap justify-between gap-2 mt-3 items-center">
              ...
              <div className="flex items-center gap-0.5">
                {[...Array(5)].map((_, i) => (
                  <svg
                    key={i}
                    className={`w-3 h-3 sm:w-[14px] sm:h-[14px] ${i < 4 ? "fill-yellow-400" : "fill-[#CED5D8]"}`}
                    viewBox="0 0 14 13"
